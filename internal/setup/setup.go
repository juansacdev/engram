...
- Save memory BEFORE composing that final answer, not after. Never let a mem_save or mem_judge be the last action in a turn that still owed the user a substantive reply.
- If a memory chain (mem_save then mem_judge) ran late in the turn, still write the full answer in that final message — do not collapse it into a one-line "saved / done" acknowledgement.
